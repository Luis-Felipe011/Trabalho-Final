...
def comparar():
    ...
    linhas = [
        ...
        ("Inicialização runner",  "Runner.run_sync(agent, task)",   "Runner(agent, app, session)"),
    ...
    loc = {
        ...
        "Instância do agente":      {"openai": "6 linhas (Agent: name, instructions, tools, model)",
                                     "adk":    "7 linhas (Agent: name, model, description, instruction, tools)"},
        "Execução do agente":       {"openai": "1 linha  (Runner.run_sync)",
  ...
  • Runner.run_sync abstrai toda a gestão de sessão — ideal para scripts rápidos.
